...
function enter_battleship_placing_ui() {
  ...
  // add buttons to user controller
  $('.controller').html(
    '<button id="next-step">提交战舰</button>'
    );
...
async function enter_playing_game_ui(player_name, index, guess_square) {
  ...
  $('.controller').html(
    '<button id="forfeit">认输</button>\
     <button id="claim_win">宣布获胜</button>\
     <button id="accuse_cheating">指责作弊</button>\
     <button id="accuse_timeout">指责超时</button>\
     <button id="handle_timeout">回应指控</button>\
     <button id="claim_timout_winnings">宣布因对方超时而获胜</button>'
    );
  ...
  // setup claim_win buttons
  $('#player1 > .controller > #claim_win').click(function () {
    console.log("click claim_win");
    claim_win('player1', 0);
  });
  $('#player2 > .controller > #claim_win').click(() => { claim_win('player2', 1); });
  // setup claim_cheat buttons
  $('#player1 > .controller > #accuse_cheating').click(function () {
    console.log("click accuse_cheating");
    accuse_cheating('player1', 0);
  });
